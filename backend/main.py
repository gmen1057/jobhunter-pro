...
@app.get("/auth/test")
async def auth_test():
    """Диагностическая страница для проверки OAuth настроек"""
    ...
    return {
        ...
        "instructions": [
            "1. Перейдите по auth_url",
            "2. Авторизуйтесь и нажмите 'Разрешить'", 
            "3. Вас перенаправит на callback_url с параметром code"
        ]
    }
...
async def hh_auth():
    ...
    redirect_uri = os.getenv("HH_REDIRECT_URI", "http://80.74.25.17:8001/auth/callback")
    auth_url = f"https://hh.ru/oauth/authorize?response_type=code&client_id={client_id}&redirect_uri={redirect_uri}"
...
async def auth_callback(request: Request, code: str = None, error: str = None, error_description: str = None):
    ...
    if error:
        ...
        error_messages = {
            ...
            'server_error': 'Ошибка сервера HeadHunter'
        }
        error_msg = error_messages.get(error, f'Ошибка авторизации: {error}')
        error_url = f"https://jhunterpro.ru/?error=oauth_error&message={error_msg}"
        from fastapi.responses import RedirectResponse
        return RedirectResponse(url=error_url)
    
    if not code:
        logger.error("No code parameter received")
        error_url = "https://jhunterpro.ru/?error=no_code&message=Код авторизации не получен"
        from fastapi.responses import RedirectResponse
        return RedirectResponse(url=error_url)
