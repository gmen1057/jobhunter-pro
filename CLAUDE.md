...
### 18. ⚠️ Обнаружена проблема с отображением пользователя (27.06.2025 - четвертая сессия)
- **Проблема**: В интерфейсе показывается тестовый аккаунт "user@example.com" вместо реальных данных пользователя
- **Причина**: Endpoint `/user/profile` возвращает моковые данные из TODO блока в коде
- **Статус**: OAuth полностью работает, токены получаются корректно, но UI показывает тестовые данные
- **Требуется**: Заменить моковые данные на реальный вызов `hh_client.get_me()` в endpoint `/user/profile`
...
- ⚠️ Отображение реальных данных пользователя (моковые данные в /user/profile)
- ⚠️ Релевантный поиск по professional_roles (в процессе)

### Приоритетные задачи:
1. Исправить endpoint `/user/profile` для отображения реальных данных пользователя
2. Реализовать релевантный поиск вакансий с использованием professional_roles из резюме
3. Добавить сохранение токенов в БД с refresh_token механизмом
